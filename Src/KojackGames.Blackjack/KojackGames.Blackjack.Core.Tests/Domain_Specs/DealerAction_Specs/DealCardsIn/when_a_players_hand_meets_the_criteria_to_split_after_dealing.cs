...
namespace KojackGames.Blackjack.Core.Tests.Domain_Specs.DealerAction_Specs.DealCardsIn
{
    ...
    public class when_a_players_hand_meets_the_criteria_to_split_after_dealing : with_OptionsAvailableToPlayerAfterDealing
    {
        private Establish context = () =>
        {
            ...
            split_spec.Stub(x => x.is_satisfied_by(playing_hand)).Return(true);
        };
        ...
        {
            split_spec.AssertWasCalled(x => x.is_satisfied_by(playing_hand));
        };

        It should_mark_the_players_hand_as_being_able_to_split_his_hand = () =>
        {
            playing_hand.AssertWasCalled(x => x.mark_as_able_to_split());
        };
