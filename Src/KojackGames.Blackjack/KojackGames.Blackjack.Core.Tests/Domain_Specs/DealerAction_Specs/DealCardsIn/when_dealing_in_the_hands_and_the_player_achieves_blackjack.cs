...
namespace KojackGames.Blackjack.Core.Tests.Domain_Specs.DealerAction_Specs.DealCardsIn
{
    ...
    public class when_dealing_in_the_hands_and_the_player_achieves_blackjack : with_a_deal_in_cards_action
    {
        ...
        It should_determine_the_winning_hands =
            () => playing_hands_end_game_status_decider.AssertWasCalled(x => x.decide_end_game_status_for_hands_in(positions));

        It should_pay_out_any_winnings_to_the_player =
            () => chip_allocator.AssertWasCalled(x => x.allocate_chips_to_player_for_winnings_hands_in(positions, player));        
    }    
}